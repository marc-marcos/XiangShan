 ...
 * @param hasContextFlush Whether the write buffer has a context-flush signal, used to invalidate all entries on context switch
...
class WriteBuffer[T <: WriteReqBundle](
  ...
  for (nRows <- 0 until numPorts) {
    ...
    when(contextFlush) {
      // context switch: invalidate all entries so stale data is not written back to a zeroed SRAM
      for (i <- 0 until numEntries) {
