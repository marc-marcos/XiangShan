...
class DecodeChannels(
  ...
  val simpleExts: Seq[ExtBase] = extensions.diff(Seq(V, Zvbb, Zvbc, Zvkg, Zvknha, Zvkned, Zacas, ZacasZabha))
  val simpleInsts = InstPattern.extensionInsts(simpleExts: _*)

  // Get vector instruction bits pattern by extensions
  val vectorExts = extensions.intersect(Seq(V, Zvbb, Zvbc, Zvkg, Zvknha, Zvkned, Zacas, ZacasZabha))
